...
if (isset($_GET['activity_id']) && isset($_GET['step_id'])) {
  $acticity = $DB->query(
    "SELECT * FROM activities WHERE activity_id = ?",
    array($_GET['activity_id'])
  );
  $acticity =  $acticity->fetch(MYSQLI_ASSOC);
  $step_id = $_GET['step_id'];
  $_SESSION['activity_id'] = $_GET['activity_id'];
  $_SESSION['step_id'] = $_GET['step_id'];
} else {
  if (isset($_SESSION['activity_id']) && isset($_SESSION['step_id'])) {
    $acticity = $DB->query(
      "SELECT * FROM activities WHERE activity_id = ?",
      array($_SESSION['activity_id'])
    );
    $acticity =  $acticity->fetch(MYSQLI_ASSOC);
    $step_id = $_SESSION['step_id'];
  } else {
    echo '<script language=javascript>
          alert(\'Pas d\'activite à modifier choisi\');
        </script> ';
  }
}

$activity_id = $acticity['activity_id'];
  ...
  ?>
    ...
    <div id="encart_choix_etape">
      ...
      <form method="post" action="modifier_activite.php">
        ...
        <input class="input_choix_etape" name="nom_activite" type="text" placeholder="nom de l'activité" value="<?= $acticity['activity_name'] ?>" required>
        ...
        ?>
        <p class="p_choix_etape"> Description :</p>
        <input class="input_choix_etape" name="description" type="text" placeholder="description de l'activité" value="<?= $acticity['description'] ?>">
        <!--Pas de vérification, l'utilisateur peut ne rien saisir-->

        <p class="p_choix_etape"> Prix :</p>
        <input class="input_choix_etape" name="price" type="number" placeholder="prix de l'activité" value="<?= $acticity['price'] ?>">
        <!--Pas de vérification, l'utilisateur peut ne rien saisir-->

        <p class="p_choix_etape">Date de début de l'activité :</p>
        <input class="input_choix_etape" name="date_d" type="date" value="<?= $acticity['date_beginning'] ?>" required>
        ...
        <input class="input_choix_etape" name="date_f" type="date" value="<?= $acticity['date_end'] ?>" required>
        <!--Message d'erreur si la date est vide-->
        <?php
        if (isset($er_date_f)) {
        ?>
        ...
        ?>
        <br>
        <button type="submit" name="creation_activite" class="button_ajouter_voyage" id="button_crea_voyage">Modifier mon activité</button>
        <div>
      </form>
    <?php
  } else {
