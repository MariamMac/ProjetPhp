...
if (!empty($_POST)) {
  ...
  if (isset($_POST['creation_activite'])) {
    ...
    $nom = (string) $_POST['select_activite'];
    ...
    if (empty($nom)) {
      $valid = false;
      $er_nom = "Il faut choisir une activité";
    }
    ...
    if ($valid) {
      if(isset($acticity['activity_id'])){
        ...
        $DB->query("INSERT INTO activities (activity_name,date_beginning,date_end,description,price,step_id) VALUES ('$nom','$date_debut','$date_fin','$description','$price', '$_SESSION[step_id]')");
      }
...
?>
  ...
  <div id="encart_choix_etape">
    <h1>Création d'activité</h1>
    <h2>Entrez votre propre activité :</h2>
    <div class="button_detail_voyage">
      <!--Permet de modifier un voyage-->
      <a href="nouvel_activite.php?step_id=<?= $_GET['step_id'] ?>">
        Je veux une activité personnalisée
      </a>
    </div>
    <br>
    <hr>
    ...
    <h2>Ou choisissez parmis les activités proposées :</h2>
    <form method="POST">
        <!--Récupération des infos à la bdd-->
<?php
        //Récupération de toutes les activités proposées à accomodation_id
        $list_activite = $DB->query('SELECT * FROM activite');
          
?>
      <select id="select_activity"  name="select_activite" required>
        <choix>Sélectionnez une activtité</choix>
        <?php
        foreach ($list_activite as $list_activite) {
        ?>
            <option name="option" value="<?=$list_activite['type']?>" >
              <?=$list_activite['type']?>
            </option>
        <?php
        }
      ...
      <!--Message d'erreur si aucune activité choisie-->
      <?php
      if (isset($er_nom)) {
      ?>
        <p><?= $er_nom ?></p>
